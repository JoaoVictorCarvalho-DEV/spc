        ...
        <div class="py-10">
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8 space-y-8">
                ...
                <div class="bg-white shadow rounded-lg">
                    <div class="flex justify-between p-6 border-b">
                        ...
                        <div>
                            <Link :href="route('devices.create')" class="flex items-center gap-2">
                                <Plus :size="16" class="text-gray-400 hover:text-gray-600 cursor-pointer" />
                    ...
                    <div class="divide-y">
                        <Link v-for="device in devices" :key="device.id" :href="route('devices.show', device.id)"
                            class="block hover:bg-gray-50 transition">
                            <div class="flex items-center justify-between p-6">
                                <div class="flex items-center gap-3">
                                    <div class="p-2 rounded-lg"
                                        :class="device.status === 'online' ? 'bg-green-50' : 'bg-gray-100'">
                                        <Power :size="20" :class="device.status === 'online' ? 'text-green-600' : 'text-gray-400'
                                            " />
                                    </div>
                                    <div>
                                ...
                                <div class="flex items-center gap-3">
                                    <span class="px-3 py-1 text-sm rounded-full inline-flex items-center gap-2" :class="device.status === 'online'
                                            ? 'bg-green-100 text-green-700'
                                            : 'bg-gray-200 text-gray-700'
                                        ">
                                        <div class="w-1.5 h-1.5 rounded-full"
                                            :class="device.status === 'online' ? 'bg-green-600' : 'bg-gray-500'"></div>
                                        {{ device.status }}
                                    </span>
                                </div>
                            </div>
